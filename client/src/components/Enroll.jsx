...
function Enroll() {
    const {
        transcript,
        listening, // Use this state for listening status
        resetTranscript,
    ...
    // Removed the redundant Listening state
    ...
    // Removed the unused handleVoiceActivation function

    useEffect(() => {
        // Update voice command state when transcript changes
        setVoiceCommand(transcript.toLowerCase().replace(/[?.;,]+$/g, ""));
    }, [transcript]);

    const handleSubmit = async (e) => { // Changed ev to e
        ...
        try {
            ...
            if (response.ok) {
                ...
                // setSelectedWebsites([]);
                // setVoiceCommand("");
                // resetTranscript(); // Reset transcript as well if needed
            } else {
                console.error("Enrollment failed:", data.message);
                // Handle error display to user if necessary
            }
            // Navigate regardless of success/fail? Or only on success? Assuming navigate anyway.
            navigate('/Landing');
        } catch (err) {
            console.error("Error occurred during submission:", err); // Changed log message for clarity
            // Handle fetch error display to user if necessary
        } finally {
             // Clear command *after* potential navigation or error handling
             setVoiceCommand("");
        }
    };
    ...
    const toggleListening = () => {
        if (listening) {
            ...
            resetTranscript(); // Often good to reset before starting a new listen
            SpeechRecognition.startListening({ continuous: true }); // Consider continuous if needed
        }
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-800 via-gray-900 to-black text-white p-6">
            {/* Popup Button */}
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-10"> {/* Added z-10 just in case */}
                <button
                    onClick={togglepopup}
            ...
            {showpopup && (
                <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
                     <motion.div
                        ...
                        className="bg-gray-100 rounded-lg p-6 max-w-md w-full shadow-xl text-gray-800" // Adjusted styling for contrast
                    >
                        <h2 className="text-xl font-semibold mb-4 text-gray-900">Executables List</h2>
                        <ul className="space-y-2 max-h-80 overflow-y-auto text-sm"> {/* Adjusted text size */}
                            {exec.map(({ name, exec }) => (
                        ...
                        <button
                            onClick={togglepopup}
                            className="mt-4 bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition duration-200 w-full" // Made button full width
                        >
                            Close
                        </button>
                    </motion.div>
            ...
            {/* Main Content */}
            <div className="flex flex-col md:flex-row w-full max-w-6xl bg-gray-800 bg-opacity-80 backdrop-blur-sm p-8 rounded-2xl shadow-2xl mt-16 md:mt-0"> {/* Added margin top for button space */}
                {/* Left Panel: App/Website Selection */}
                                ...
                                {selectedApps.map((app, index) => (
                                    <li key={index} className="p-2 flex justify-between items-center bg-gray-600 rounded-md">
                                        ...
                                        <button
                                            ...
                                            className="text-red-400 hover:text-red-500 transition text-xl font-bold ml-2" // Adjusted styling
                                            aria-label={`Remove ${app}`}
                                        >
                                            × {/* More standard remove icon */}
                                        </button>
                                    </li>
                                ))}
                            </ul>
                                ...
                                {filteredWebsites.length > 0 ? (
                                    filteredWebsites.map((site) => (
                                        <button
                                            ...
                                            className="block w-full text-left p-2 hover:bg-gray-500 transition rounded-md truncate" // Added truncate
                                        >
                                            {site}
                                        </button>
                                    ))
                                ) : ( // Allow adding custom website if search term doesn't match existing
                                    <button
                        ...
                        {selectedWebsites.length > 0 && (
                            <ul className="mt-4 space-y-2">
                                ...
                                {selectedWebsites.map((site, index) => (
                                    <li key={index} className="p-2 flex justify-between items-center bg-gray-600 rounded-md">
                                        <span className="truncate flex-1 mr-2">{index + 1}. {site}</span> {/* Added truncate */}
                                        <button
                                            onClick={() => removeItem(site, setSelectedWebsites)}
                                            className="text-red-400 hover:text-red-500 transition text-xl font-bold ml-2 flex-shrink-0" // Adjusted styling
                ...
                <div className="w-full md:w-1/2 flex flex-col items-center mt-6 md:mt-0">
                    ...
                    <motion.button // Changed div to button for better accessibility
                    ...
                    <div className="mt-8 w-full">
                        ...
                        <textarea
                            ...
                            className="w-full p-4 bg-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none" // Added resize-none
                            rows="4" // Slightly larger
                            placeholder="Speak or type your command..."
                            value={voiceCommand}
                            onChange={(e) => {
                                setVoiceCommand(e.target.value);
                                // Optional: If user types, stop listening?
                                // if (listening) SpeechRecognition.stopListening();
                            }}
                        />
                        {/* Removed redundant Stop/Start buttons as mic button handles it */}
                        <div className="flex justify-center mt-4">
                             <button
                                onClick={resetTranscript}
                                disabled={!transcript && !voiceCommand} // Disable if nothing to reset
                    ...
                    {/* Submit Button */}
                    <motion.button
                        className="mt-10 px-8 py-3 bg-blue-600 rounded-lg hover:bg-blue-700 transition shadow-lg text-lg font-semibold" // Enhanced styling
                        whileHover={{ scale: 1.05, boxShadow: "0px 0px 12px rgba(59, 130, 246, 0.5)" }} // Added hover shadow
                        onClick={handleSubmit}
                        disabled={!voiceCommand.trim()} // Disable if command is empty
                    >
                        Enroll Command
                    </motion.button>
                </div>
